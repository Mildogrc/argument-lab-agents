...
def _build_json_payload(state: DebateState, session_id: str) -> dict:
    ...
    hallucination_flags = _serialise_flags(state.get("hallucination_flags", []))
    contradiction_flags = _serialise_flags(state.get("contradiction_flags", []))
...
def _serialise_flags(flags: list) -> list[dict]:
    serialised = []
    for flag in flags:
        if hasattr(flag, "model_dump"):
            serialised.append(flag.model_dump())
        elif isinstance(flag, dict):
            serialised.append(flag)
        else:
            serialised.append({"claim_id": str(flag)})

    return sorted(
        serialised,
        key=lambda flag: (
            str(flag.get("claim_id", "")),
            str(flag.get("prior_claim_id", "")),
            str(flag.get("severity", "")),
            str(flag.get("contradiction_type", "")),
        ),
    )


def _flag_claim_ids(flags: list[dict]) -> list[str]:
    return [str(flag.get("claim_id", "")) for flag in flags if flag.get("claim_id")]
...
def _render_markdown(p: dict) -> list[str]:
    ...
    if p.get("verdict"):
        ...
        w("## Final Verdict")
        w("")
        w(f"**Type:** `{verdict.get('verdict_type')}`  ")
        w(f"**Winner:** `{verdict.get('winning_agent')}`  ")
        w("")
        w(verdict.get("summary", ""))
        w("")
        if verdict.get("justification"):
            w(f"**Justification:** {verdict['justification']}")
            w("")
        if verdict.get("unresolved_claims"):
            w("**Unresolved claims**")
            for claim in verdict["unresolved_claims"]:
                w(f"- {claim}")
            w("")
    ...
    if eval_data.get("hallucination_flags"):
        claim_ids = _flag_claim_ids(eval_data["hallucination_flags"])
        w(f"**Hallucinated claim IDs:** `{'`, `'.join(claim_ids)}`")
        w("")
    if eval_data.get("contradiction_flags"):
        claim_ids = _flag_claim_ids(eval_data["contradiction_flags"])
        w(f"**Contradicted claim IDs:** `{'`, `'.join(claim_ids)}`")
        w("")
    ...
    for round_data in p.get("rounds", []):
        ...
        for role in ("proponent", "opponent"):
            ...
            if arg.get("evidence"):
                ...
                for e in arg["evidence"]:
                    w(
                        f"- `[{e['source_id']}]` (reliability: {e['reliability_score']:.2f})"
                    )
                    w(f"  > {e['excerpt']}")
                w("")
            ...
            if arg.get("counterpoints_addressed"):
                w(
                    f"**Counterpoints addressed:** `{'`, `'.join(arg['counterpoints_addressed'])}`"
                )
                w("")
        ...
        if judge:
            ...
            w(
                f"| **Composite** | **{p_b['composite']:.3f}** | **{o_b['composite']:.3f}** |"
            )
            w("")

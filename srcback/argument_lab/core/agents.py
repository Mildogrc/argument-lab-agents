...
def _generate_argument(
    *,
    ...
    argument = argument.model_copy(
        update={
            "evidence": [
                e for e in argument.evidence if e.source_id in valid_source_ids
            ]
            or evidence_refs[
                :1
            ],  # guarantee min_length=1 even if LLM cited nothing valid
        }
    )
...
def _enforce_counterpoint_rule(
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                AGENT_SYSTEM_TEMPLATE.format_map(
                    {
                        "role": role,
                        "stance": "FOR" if role == "Proponent" else "AGAINST",
                        "proposition": proposition,
                        "counterpoint_rule": COUNTERPOINT_RULES[
                            min(current_round, MAX_ROUNDS)
                        ],
                        "evidence_context": evidence_context,
                    }
                ),
            ...
            (
                "user",
                AGENT_USER_TEMPLATE.format_map(
                    {
                        "history": history,
                        "current_round": current_round,
...
def _update_state_from_argument(
    ...
    prior_opponent_ids = {
        arg.id for arg in state.get("arguments", []) if arg.agent == opponent_role
    }
    newly_addressed = set(argument.counterpoints_addressed)
    newly_ignored = (
        prior_opponent_ids - newly_addressed - state.get("ignored_claims", set())
    )

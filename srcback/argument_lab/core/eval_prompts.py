...
def format_prior_scores(scores: list[JudgeEvaluation]) -> str:
    ...
    for s in scores:
        ...
        lines.append(f"  Round {s.round}: " f"Proponent={p:.3f}  Opponent={o:.3f}")
    return "\n".join(lines)
...
# ---------------------------------------------------------------------------
# Final verdict prompts
# ---------------------------------------------------------------------------

VERDICT_SYSTEM = """\
You are the final judge for a completed multi-round AI debate.
Your job is to synthesize the full debate into a structured verdict.

Choose verdict_type:
  consensus      The agents reached a meaningful shared conclusion.
  best_argument  One agent made the stronger overall case.
  stalemate      The debate ended without a clear winner or consensus.

Choose winning_agent:
  proponent      The proponent made the stronger case.
  opponent       The opponent made the stronger case.
  none           Use for consensus or stalemate when no side clearly wins.

Base your decision on the debate history, judge score trajectory, unresolved
claims, and whether later rounds addressed earlier weaknesses.

Return only the required JSON schema. No prose outside the schema.
"""
...
Produce the final structured verdict. Include:
  - verdict_type
  - winning_agent
  - summary
  - unresolved_claims
